...
from risk import positions as pos
from risk import utils as ut
from risk import db_sql as db
from tools.charting import charting as CH
from tools.positions import fetch_db as fdb
from tools.positions.fetch_cust import rich_portfolio


def garch_exp(rets_rescaled, dist='skewt'):
    """
...
def calculate_vaR_garch(rets, window, horizon, rescale, decay, ftol, max_iter, garch_engine):
    """
    ...
    if rets.index.dtype == 'O':
        rets = rets.rename(index=lambda x: datetime(x.year, x.month, x.day))

    # Apply Sample Size
    rets_garch = rets[-window:].copy()
    # Use decay
    if decay != 1: rets_garch = rets_garch.sort_index(ascending=False).ewm(alpha=decay, adjust=True).mean()
    ...
    forecasts = res.forecast(horizon=horizon, start=str(rets_garch.index[0])[:10], simulations=1000, reindex = False)
    cond_mean = forecasts.mean[-horizon:]
    cond_var = forecasts.variance[-horizon:]
    q = am.distribution.ppf([0.05, 0.01], res.params[-2 if garch_engine == 'egarch' else -1:])
    value_at_risk = -cond_mean.values - np.sqrt(cond_var).values * q[None, :]
    value_at_risk = pd.DataFrame(value_at_risk, columns=['VaR95_1D', 'VaR99_1D'], index=cond_var.index)

    # Scale for 20 Trading Day VaR
    value_at_risk['VaR95_20D'] = value_at_risk['VaR95_1D'] * np.sqrt(20)
    value_at_risk['VaR99_20D'] = value_at_risk['VaR99_1D'] * np.sqrt(20)
